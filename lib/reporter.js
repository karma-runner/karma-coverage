...
var CoverageReporter = function (rootConfig, helper, logger, emitter) {
  ...
  function checkCoverage (browser, coverageMap) {
    ...
    var emitWarning = config.check.emitWarning
    ...
    function check (name, thresholds, actuals, emitWarning) {
      var keys = [
        'statements',
        'branches',
        'lines',
      ...
      keys.forEach(function (key) {
        ...
        if (threshold < 0) {
          if (threshold * -1 < actualUncovered) {
            var exceedingThresholdMessage = browser.name + ': Uncovered count for ' + key + ' (' + actualUncovered +
              ') exceeds ' + name + ' threshold (' + -1 * threshold + ')'
            if (emitWarning) {
              log.warn(exceedingThresholdMessage)
            } else {
              coverageFailed = true
              log.error(exceedingThresholdMessage)
            }
          }
        } else {
          if (actual < threshold) {
            var thresholdMessage = browser.name + ': Coverage for ' + key + ' (' + actual +
              '%) does not meet ' + name + ' threshold (' + threshold + '%)'
            if (emitWarning) {
              log.warn(thresholdMessage)
            } else {
              coverageFailed = true
              log.error(thresholdMessage)
            }
          }
        }
      })
    ...
    check('global', thresholds.global, globalResults.toJSON(), emitWarning)
    ...
    eachTrackedFiles.forEach(function (key) {
      ...
      check('per-file' + ' (' + key + ') ', keyThreshold, eachResults[key].toJSON(), emitWarning)
    })

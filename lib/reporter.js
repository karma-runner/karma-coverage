...
var CoverageReporter = function(rootConfig, helper, logger) {
  ...
  this.onRunComplete = function(browsers) {
    ...
    reporters.forEach(function(reporterConfig) {
      browsers.forEach(function(browser) {
        ...
        if (collector) {
          // stash coverageData
          if (collector.files) {
            coverageData[browser.id] = {};
            collector.files().forEach(function (key) {
              coverageData[browser.id][key] = collector.fileCoverageFor(key);
            });
          }

          if (reporterConfig.type !== 'none') {
              pendingFileWritings++;

            outputDir = helper.normalizeWinPath(path.resolve(basePath,
              generateOutputDir(browser.name,
                reporterConfig.dir || config.dir,
                reporterConfig.subdir || config.subdir
              )));

            options = helper.merge({}, reporterConfig, {
              dir : outputDir,
            ...
            reporter = istanbul.Report.create(reporterConfig.type || 'html', options);
            // If reporting to console, skip directory creation
            if (reporterConfig.type && reporterConfig.type.match(/^(text|text-summary)$/) && typeof
              reporterConfig.file === 'undefined') {
    ...
    // add coverage data to root config
    rootConfig.coverageReporter = rootConfig.coverageReporter || {};
    rootConfig.coverageReporter.coverageData = coverageData;
  };
